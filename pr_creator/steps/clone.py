...
def _get_branch_to_checkout(
    ...
    try:
        ...
        try:
            ...
            logger.info("Found existing branch %s for change_id %s", branch_name, change_id)
            return branch_name
        except Exception:
            logger.info("Branch %s does not exist yet for change_id %s", branch_name, change_id)
            return None
...
def clone_repo(
    ...
    token = os.environ.get("GITHUB_TOKEN")
    branch_to_checkout = _get_branch_to_checkout(repo_url, change_id, branch_prefix, token)
    
    if branch_to_checkout:
        # Branch exists on remote - clone normally first, then checkout the branch
        logger.info("Cloning %s -> %s (checking out branch %s)", repo_url, target, branch_to_checkout)
        ...
        if remote_ref in repo.refs:
            ...
            if branch_ref not in repo.refs:
                porcelain.branch_create(repo.path, branch_to_checkout.encode(), repo.refs[remote_ref])
            ...
            logger.warning("Remote branch %s not found, falling back to %s", branch_to_checkout, default_branch)
        ...
        default_branch = _get_default_branch(repo_url, token)
        logger.info("Cloning %s -> %s (checking out %s)", repo_url, target, default_branch)
        porcelain.clone(clone_url, target, checkout=True)
...
def _branch_exists_remotely(repo_url: str, change_id: Optional[str]) -> bool:
    ...
    branch_prefix = os.environ.get("SUBMIT_BRANCH_PREFIX", "auto/pr")
    token = os.environ.get("GITHUB_TOKEN")
    return _get_branch_to_checkout(repo_url, change_id, branch_prefix, token) is not None
